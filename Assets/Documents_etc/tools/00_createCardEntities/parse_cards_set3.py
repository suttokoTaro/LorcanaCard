...
# 入力ファイルと出力ファイルのパス
input_html_path = "inputCards_set3.html"
output_csv_path = "00_output_set3.csv"
...
def parse_card(card_div, card_id):
    ...
    return {
        ...
        "inkwellFlag": 1, #（1 if "インクあり" in info.get("インク", "") else 0,
        "vanillaFlag": 1 if effect_text == "-" else 0,
        "bodyguardFlag": 2 if "護衛" in effect_text else 0,
        "challengerFlag": 2 if "果敢" in effect_text else 0,
        "evasiveFlag": 2 if "回避" in effect_text else 0,
        "recklessFlag": 2 if "暴勇" in effect_text else 0,
        "resistFlag": 2 if "耐久" in effect_text else 0,
        "rushFlag": 2 if "突進" in effect_text else 0,
        "shiftFlag": 2 if "変身" in effect_text else 0,
        "singerFlag": 2 if "歌声" in effect_text else 0,
        "singTogetherFlag": 2 if "合唱" in effect_text else 0,
        "supportFlag": 2 if "支援" in effect_text else 0,
        "wardFlag": 2 if "魔除" in effect_text else 0,
...
def main():
    ...
    start_card_id = 3001  # ← ここで初期 cardId を指定
